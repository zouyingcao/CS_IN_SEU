...
void CImageProcessingView::OnTranslation()
{
	...
	if (dlg.DoModal() == IDOK) //显示对话框
	{
		...
		CGeometricTransform<BYTE>  m_rImage;
		m_rImage.Translation(dlg.translationX, dlg.translationY, &(pDoc->m_sImage));
		CDC* pdc = this->GetDC();
		RedrawWindow();
		m_rImage.Draw(pdc->m_hDC,500,0);// 不要求保留处理结果
		ReleaseDC(pdc);
	}
}
...
void CImageProcessingView::OnScaling()
{
	...
	{
		CGeometricTransform<BYTE>  m_rImage;
		m_rImage.Scaling(dlg.ratioX, dlg.ratioY,&(pDoc->m_sImage));
		CDC* pdc = this->GetDC();
		RedrawWindow();
		m_rImage.Draw(pdc->m_hDC, 500, 0);// 不要求保留处理结果
		ReleaseDC(pdc);
...
void CImageProcessingView::OnRotation()
{
	...
	{
		CGeometricTransform<BYTE>  m_rImage;
		m_rImage.Rotate(dlg.degree,dlg.rotateX,dlg.rotateY, &(pDoc->m_sImage));
		CDC* pdc = this->GetDC();
		RedrawWindow();
		m_rImage.Draw(pdc->m_hDC, 500, 0);// 不要求保留处理结果
		ReleaseDC(pdc);
