...
BEGIN_MESSAGE_MAP(CImageProcessingView, CScrollView)
	...
	ON_COMMAND(ID_32777, &CImageProcessingView::OnRegionGrow)
	ON_COMMAND(ID_RegionGrow, &CImageProcessingView::OnRegionGrow)
	ON_COMMAND(ID_GlobalThreshold, &CImageProcessingView::OnGlobalThreshold)
	ON_COMMAND(ID_BlockingThreshold, &CImageProcessingView::OnBlockingthreshold)
END_MESSAGE_MAP()
...
void CImageProcessingView::OnDraw(CDC* pDC)
{
	...
	// 分割后的图像从原始图像右边100个像素的位置开始显示
	if (pDoc->segmentedImage.GetData() != NULL)
		(pDoc->segmentedImage).Draw(pDC->m_hDC, pDoc->m_sImage.GetWidth() + 100, 0);
}
...
void CImageProcessingView::OnInitialUpdate()
{
	...
	sizeTotal.cx = 4000; sizeTotal.cy = 4000;
...
void CImageProcessingView::OnRegionGrow()
{
	...
	//pDoc->segmentedImage.RegionGrow(&(pDoc->m_sImage), (BYTE)(6));	// typeid(pDoc->m_sImage.GetData())
	Invalidate();
}


void CImageProcessingView::OnGlobalThreshold()
{
	// TODO: 在此添加命令处理程序代码
	CImageProcessingDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	...
	pDoc->segmentedImage.GlobalThreshold(&(pDoc->m_sImage));
	Invalidate();
}


void CImageProcessingView::OnBlockingthreshold()
{
	// TODO: 在此添加命令处理程序代码
	CImageProcessingDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	if (!pDoc)
		return;
	CBlockingThresholdDlg dlg;
	if (dlg.DoModal() == IDOK) //显示对话框
	{
		pDoc->segmentedImage.BlockingThreshold(&(pDoc->m_sImage),dlg.row,dlg.col);
		Invalidate();
	}
}
