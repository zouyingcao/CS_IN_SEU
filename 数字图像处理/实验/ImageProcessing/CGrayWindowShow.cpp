...
void CGrayWindowShow<T_DATA>::GrayMapping(int windowWidth,int windowLevel,CImageData<unsigned short>*img)
{
	...
	for (int j = 0; j < mHeight; j++)
		for (int i = 0; i < mWidth; i++)
		{
			...
			double upper = (double)windowLevel + (double)windowWidth / 2;
			double lower = (double)windowLevel - (double)windowWidth / 2;
			if (oldPixel > upper)
				mpPixels[j * mWidth + i] = (T_DATA)255;
			else if (oldPixel < lower)
				mpPixels[j * mWidth + i] = (T_DATA)0;
			else
				mpPixels[j * mWidth + i] = (T_DATA)(255 * (oldPixel - lower) / (double)windowWidth);
		}
}