    ...
    for (int i = 0; i < mHeight; i++)
        for (int j = 0; j < mWidth; j++) {
            if (i == 0 || j == 0 || i == mHeight - 1 || j == mWidth - 1)
                ...
                //secondDerivative = *(img->GetData() + (i + 1) * mWidth + j) + *(img->GetData() + (i - 1) * mWidth + j) + *(img->GetData() + i * mWidth + j + 1) + *(img->GetData() + i * mWidth + j - 1) + *(img->GetData() + (i + 1) * mWidth + j + 1) + *(img->GetData() + (i + 1) * mWidth + j - 1) + *(img->GetData() + (i - 1) * mWidth + j + 1) + *(img->GetData() + (i - 1) * mWidth + j - 1) - 8 * (*(img->GetData() + i * mWidth + j));
            //*(mpPixels + i * mWidth + j) = *(pOldBits + i * mWidth + j) - k * secondDerivative;
