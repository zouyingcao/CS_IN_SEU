...
template <class T_DATA>
void CImageData<T_DATA>::LoadFromRaw(CString& imagePath)
{	
	ifstream inFile(imagePath, ios::binary | ios::in);
	if (!inFile) {
		AfxMessageBox(_T("file cannot open!"));
		exit(1);
	}
	inFile.read((char*)(&mWidth), sizeof(unsigned long));
	inFile.read((char*)(&mHeight), sizeof(unsigned long));
	mpPixels = new T_DATA[mHeight * mWidth];

	for (int i = 0; i < mWidth * mHeight; i++)
	{
		inFile.read((char*)(mpPixels + i), sizeof(T_DATA));
	}
	/*
	FILE* fp;
	fopen_s(&fp,, "r");
	if (fp == NULL)
		exit(1);
	fread(&mWidth, sizeof(unsigned long), 1, fp);
	fread(&mHeight, sizeof(unsigned long), 1, fp);
	mpPixels = new T_DATA[mHeight * mWidth];
	while (!feof(fp))
		fread(mpPixels, sizeof(T_DATA), 1, fp);
	fclose(fp);
	*/
}
...
void CImageData<T_DATA>::Draw(HDC hDC, int x, int y)
{
	if (mpPixels == NULL)
		...
		if (sizeof(T_DATA) == 1) {
			...
			for (int i = 0; i < MaxColors; i++)
			{
				ColorTable[i].rgbBlue = (T_DATA)i;
				ColorTable[i].rgbGreen = (T_DATA)i;
				ColorTable[i].rgbRed = (T_DATA)i;
			}
			mpImage.SetColorTable(0, MaxColors, ColorTable);
			delete[]ColorTable;
		}
		...
		if (MaxVal > MinVal)
		{
			for (int y = 0; y < mHeight; y++)
			{
				for (int x = 0; x < mWidth; x++)
				{
					// 先归一化后到0-255之间
					*(pData + x) = (*(mpPixels + y * mWidth + x) - MinVal) * 255 / (MaxVal - MinVal);
					//*(pData + x) = *(mpPixels + y * mWidth + x);
				}
				pData += rowBytes / sizeof(T_DATA);
			}
		}
